...
const ChartTerminal = function ChartTerminal({
  assetId,
  candles,
  marketStructure,
  showAIOverlay,
  ...
  useEffect(() => {
    ...
    const renderChart = () => {
      ...
      if (activeDrawStart && crosshairRef.current) {
      ...
      if (crosshairRef.current) {
        ...
        // Horizonal
        ctx.beginPath();
        ctx.moveTo(0, crosshairRef.current!.y);
        ctx.lineTo(width - 70, crosshairRef.current!.y);
        ctx.stroke();

        // Vertical
        ctx.beginPath();
        ctx.moveTo(crosshairRef.current!.x, 0);
        ctx.lineTo(crosshairRef.current!.x, height);
        ctx.stroke();
        ctx.setLineDash([]);

        // Horizonal axis marker price
        const crosshairPrice = maxPrice - ((crosshairRef.current!.y - 30) / (height - 80)) * (maxPrice - minPrice);
        ctx.fillStyle = "#0d0d10";
        ctx.fillRect(width - 70, crosshairRef.current!.y - 8, 70, 16);
        ctx.strokeStyle = "rgba(212, 175, 55, 0.4)";
        ctx.lineWidth = 1;
        ctx.strokeRect(width - 70, crosshairRef.current!.y - 8, 70, 16);
        ctx.fillStyle = "#D4AF37";
        ctx.font = "bold 8px 'JetBrains Mono', monospace";
        ctx.fillText(
  ...
  }, [candles, marketStructure, zoomFactor, panOffset, crosshairRef.current, activeOverlays, drawnLines, activeDrawStart, selectedTool, assetId, aiAnalysis]);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (x > rect.width - 70) {
      if (crosshairRef.current !== null) { crosshairRef.current = null; if (renderChartRef.current) requestAnimationFrame(renderChartRef.current); }
      return;
    }

    crosshairRef.current = { x, y }; if (renderChartRef.current) requestAnimationFrame(renderChartRef.current);
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (selectedTool === "trendline" && crosshairRef.current) {
      if (!activeDrawStart) {
        setActiveDrawStart({ x: crosshairRef.current!.x, y: crosshairRef.current!.y });
      } else {
        setDrawnLines([
          ...drawnLines,
          { x1: activeDrawStart.x, y1: activeDrawStart.y, x2: crosshairRef.current!.x, y2: crosshairRef.current!.y },
        ]);
        setActiveDrawStart(null);
      }
    }
...
export default React.memo(ChartTerminal);
